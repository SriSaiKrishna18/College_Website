// ═══════════════════════════════════════════════════════════════════════════
//  SPARK Student Portal - Main Application Script
//  Modern, Feature-rich Student Portal
// ═══════════════════════════════════════════════════════════════════════════
...
function initApp() {
    ...
    currentStudentId = sessionStorage.getItem('studentId') || localStorage.getItem('studentId');
    
    if (!currentStudentId) {
        // Use a default student for demo purposes
...
function initDashboard() {
    ...
    dashboardItems.forEach(item => {
        item.addEventListener('click', function() {
            ...
            // Show the target content section with fade in
            ...
            if (targetSection) {
                ...
                if (targetPage === 'attendance' && currentStudent.student_id) {
                    loadAttendanceData(currentStudent.student_id);
                } else if (targetPage === 'courses' && currentStudent.student_id) {
                    loadCoursesData(currentStudent.student_id);
                } else if (targetPage === 'results' && currentStudent.student_id) {
                    loadResultsData(currentStudent.student_id);
                } else if (targetPage === 'analysis' && currentStudent.student_id) {
...
function updateStudentProfile(student) {
    ...
    if (profileName) profileName.textContent = student.name || 'Student';
    if (profileYear) profileYear.textContent = `${student.branch || 'Department'} - Year ${Math.ceil((student.current_semester || 1) / 2)}`;
    ...
    if (cgpaElement) cgpaElement.textContent = student.cgpa || '0.00';
    
    // Update profile page details
    const profileElements = {
        '.student-name': student.name,
        '.student-roll': student.roll_number,
